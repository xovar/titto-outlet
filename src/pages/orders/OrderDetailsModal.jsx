...
export default function OrderDetailsModal({ isOpen, onClose, order }) {
  // ESC Key চাপলে মোডাল বন্ধ হওয়ার লজিক এবং বডি স্ক্রল অফ রাখা
  useEffect(() => {
  ...
  // React Portal দিয়ে সরাসরি body তে রেন্ডার করা হচ্ছে
  return createPortal(
    <div className="fixed inset-0 z-99999 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      ...
      >
            ...
            {order.items && order.items.length > 0 ? (
              <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-200/70 dark:border-slate-800 rounded-xl overflow-hidden">
                {order.items.map((item, idx) => (
                  <div key={idx} className="p-3.5 flex items-center justify-between text-xs bg-white dark:bg-slate-900">
        ...
        {/* Modal Footer */}
        <div className="px-6 py-3.5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 flex justify-end shrink-0">
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-lg text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          >
            Close
          </button>
        </div>
